...
async function buildHwpx(d){
  ...
  const tpl=await fetch('assets/base.hwpx.b64'); if(!tpl.ok) throw new Error('assets/base.hwpx.b64를 불러오지 못했습니다. GitHub Pages 또는 간단한 웹서버에서 실행하세요.');
  const b64=(await tpl.text()).trim();
  const raw=atob(b64);
